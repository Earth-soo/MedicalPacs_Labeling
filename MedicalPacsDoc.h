
// MedicalPacsDoc.h: CMedicalPacsDoc 클래스의 인터페이스
//


#pragma once
#include "CPacs.h"

class CMedicalPacsDoc : public CDocument
{
protected: // serialization에서만 만들어집니다.
	CMedicalPacsDoc() noexcept;
	DECLARE_DYNCREATE(CMedicalPacsDoc)

// 특성입니다.
public:
	shared_ptr<CPacs> m_pPacs;
// 작업입니다.
public:

// 재정의입니다.
public:
	virtual BOOL OnNewDocument();
	virtual BOOL OnOpenDocument(LPCTSTR lpszPathName);
	virtual void Serialize(CArchive& ar);
#ifdef SHARED_HANDLERS
	virtual void InitializeSearchContent();
	virtual void OnDrawThumbnail(CDC& dc, LPRECT lprcBounds);
#endif // SHARED_HANDLERS

// 구현입니다.
public:
	virtual ~CMedicalPacsDoc();
#ifdef _DEBUG
	virtual void AssertValid() const;
	virtual void Dump(CDumpContext& dc) const;
#endif

protected:

// 생성된 메시지 맵 함수
protected:
	DECLARE_MESSAGE_MAP()

#ifdef SHARED_HANDLERS
	// 검색 처리기에 대한 검색 콘텐츠를 설정하는 도우미 함수
	void SetSearchContent(const CString& value);
#endif // SHARED_HANDLERS
public:
	virtual void OnCloseDocument();
	afx_msg void OnHistogram();
	afx_msg void OnGaussian();

	afx_msg void OnRoverts();
	afx_msg void OnEdgePrewitt();
	afx_msg void OnEdgeSobel();
	afx_msg void OnEdgeColor();
	afx_msg void OnEdgeCanny();
	afx_msg void OnImageReset();
	afx_msg void OnSharpening();
	afx_msg void OnSegmentBinarization();
	afx_msg void OnContourTracing();
	
	afx_msg void OnLabeling();
};
